...
import com.fitmate.app.ui.theme.ThemeState
    ...
    private fun loadEmail() {
        viewModelScope.launch {
            authRepo.authState.collect { state ->
                if (state is com.fitmate.app.domain.model.AuthState.Authenticated) {
    ...
    fun saveProfile() {
        viewModelScope.launch {
            ...
            val updated = current.copy(
                ...
                gender = com.fitmate.app.domain.model.Gender.entries.find { it.name.lowercase() == s.formGender } ?: current.gender,
                activityLevel = com.fitmate.app.domain.model.ActivityLevel.entries.find { it.name.lowercase() == s.formActivityLevel } ?: current.activityLevel,
                fitnessGoals = s.formGoals.mapNotNull { g -> com.fitmate.app.domain.model.FitnessGoal.entries.find { it.name.lowercase() == g } },
                ...
                liftingExperience = runCatching { com.fitmate.app.domain.model.LiftingExperience.valueOf(s.formLiftingExperience) }.getOrDefault(current.liftingExperience),
                trainingLocation = runCatching { com.fitmate.app.domain.model.TrainingLocation.valueOf(s.formTrainingLocation) }.getOrDefault(current.trainingLocation),
                unitSystem = runCatching { com.fitmate.app.domain.model.UnitSystem.valueOf(s.formUnitSystem) }.getOrDefault(current.unitSystem)
            )
